...
-- User roles enum - Updated to include all required roles
DO $$ BEGIN
    -- Drop existing enum if it exists to recreate with correct roles
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role_enum') THEN
        -- First update any existing columns to use text temporarily
...
-- =====================================================
-- 14.5. DATA MIGRATION FOR USER ROLES
-- =====================================================

-- Update existing users to use the new role enum
-- This handles the transition from old roles to new roles
UPDATE users SET role = 'admin' WHERE role::text IN ('super_admin', 'admin');
UPDATE users SET role = 'manager' WHERE role::text = 'manager';
UPDATE users SET role = 'staff' WHERE role::text = 'staff';
UPDATE users SET role = 'guest' WHERE role::text = 'guest';

-- Add any missing users with default roles
...
-- Create a view for role-based analytics
