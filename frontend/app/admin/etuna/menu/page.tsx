...
export const metadata: Metadata = {
  ...
  description: 'Menu management dashboard showcasing Buffr Host platform capabilities for Etuna Guesthouse',
};
...
export default function EtunaMenuPage() {
  ...
  const categories = [
    ...
    { name: 'Traditional Cuisine', count: menuItems.filter(item => item.category === 'Traditional Cuisine').length },
    ...
    { name: 'Light Meals', count: menuItems.filter(item => item.category === 'Light Meals').length },
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-base-content/70 mt-2">Manage restaurant menu items and pricing</p>
          </div>
        ...
        <div className="card-body">
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <button
                key={category.name}
                className={`btn btn-sm ${category.name === 'All' ? 'btn-primary' : 'btn-outline'}`}
              >
        ...
        {menuItems.map((item) => (
          <div key={item.id} className="card bg-base-100 shadow-xl">
            <figure className="h-48 bg-gray-200">
              <Image className="w-full h-full object-cover" alt={item.name} />
            </figure>
            <div className="card-body">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className={`badge ${getAvailabilityColor(item.availability)}`}>
                    {item.availability}
                  </div>
                </div>
              </div>
              ...
              .map((item, index) => (
                <div key={item.id} className="flex items-center justify-between p-3 bg-base-200 rounded-lg">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm text-base-content/70">NAD {item.revenue.toLocaleString()}</div>
                  </div>
                </div>
              ))}
          </div>
