  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const analyticsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/analytics/dashboard`, {
          headers: {
        ...
        // Fetch recent orders
        const ordersResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders?limit=5&status=all`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {isLoading ? (
                    ...
                    Array.from({ length: 4 }).map((_, index) => (
                      <div key={index} className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 animate-pulse" role="status" aria-label="Loading analytics data">
                ...
                [
                  {
                    title: 'Total Revenue',
                    value: `N$${stats.total_revenue?.toLocaleString() || '0'}`,
                ...
                <table className="table table-zebra w-full">
                  ...
                  <tbody>
                        {isLoading ? (
                          ...
                          Array.from({ length: 4 }).map((_, index) => (
                            <tr key={index} role="status" aria-label="Loading orders">
                              <td><div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div></td>
                              <td><div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div></td>
                              <td><div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div></td>
                              <td><div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div></td>
                              <td><div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div></td>
              ...
              <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400">Customer John Doe registered</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Inventory low on item #456</span>
                  </div>
                </div>
              </div>
