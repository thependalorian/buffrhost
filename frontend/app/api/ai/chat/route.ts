...
Be helpful, professional, and culturally sensitive. Always encourage direct contact for bookings.`
    ...
    const aiResponse = data.choices[0]?.message?.content || 'I apologize, but I cannot process your request at the moment. Please contact our reception at +264 65 231 177.';
    ...
    if (['emergency', 'urgent', 'help', 'medical', 'fire', 'accident'].some(word => lowerMessage.includes(word))) {
      conversationType = 'emergency';
      priority = 'urgent';
    } else if (['book', 'reservation', 'room', 'stay'].some(word => lowerMessage.includes(word))) {
      conversationType = 'booking';
      priority = 'medium';
    } else if (['complaint', 'problem', 'issue'].some(word => lowerMessage.includes(word))) {
    ...
    const result = {
      ...
      conversationType,
      priority,
      requiresHandoff: priority === 'urgent' || conversationType === 'emergency',
      turnCount: 1,
      sessionId: sessionId || `session_${new Date().toISOString()}`,
      safetyNotice: priority === 'urgent' ? 'Please contact reception immediately at +264 65 231 177' : undefined
    };
