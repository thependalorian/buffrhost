...
const BenefitEnrollmentForm: React.FC<BenefitEnrollmentFormProps> = ({
  employeeId,
  initialData,
  onSubmit,
}) => {
  const [formData, setFormData] = useState<BenefitEnrollmentFormData>(
    initialData || {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "employee_contribution" || name === "company_contribution"
          ? parseFloat(value)
          : value,
    }));
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="benefit-enrollment-form">
      <h3>
        {initialData ? "Edit" : "Add"} Benefit Enrollment for Employee{" "}
        {employeeId}
      </h3>
      <div>
