...
const PayrollRecordList: React.FC<PayrollRecordListProps> = ({
  employeeId,
}) => {
    ...
    <div className="payroll-record-list">
      ...
      <ul>
        {payrollRecords.map((record) => (
          <li key={record.id}>
            {record.pay_period_start} to {record.pay_period_end}: Gross{" "}
            {record.gross_pay}, Net {record.net_pay} ({record.status})
          </li>
        ))}
      </ul>
    </div>
