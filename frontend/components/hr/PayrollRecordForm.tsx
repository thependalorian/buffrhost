...
const PayrollRecordForm: React.FC<PayrollRecordFormProps> = ({
  employeeId,
  initialData,
  onSubmit,
}) => {
  ...
  const handleChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "gross_pay" || name === "net_pay" || name === "bonuses"
          ? parseFloat(value)
          : value,
    }));
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="payroll-record-form">
      <h3>
        {initialData ? "Edit" : "Add"} Payroll Record for Employee {employeeId}
      </h3>
      <div>
