...
// Find all files that import old components
const findFiles = () => {
  try {
    const result = execSync(
      'grep -r "@/components/ui/\\(card\\|button\\|input\\|label\\|tabs\\)" --include="*.tsx" --include="*.ts" .',
      {
    ...
    const replacements = [
      {
        ...
        to: 'BuffrCard, BuffrCardBody, BuffrCardHeader, BuffrCardTitle',
      },
      { from: '@/components/ui/button', to: 'BuffrButton' },
      { from: '@/components/ui/input', to: 'BuffrInput' },
      { from: '@/components/ui/label', to: 'BuffrLabel' },
      {
        from: '@/components/ui/tabs',
        to: 'BuffrTabs, BuffrTabsContent, BuffrTabsList, BuffrTabsTrigger',
      },
    ];
