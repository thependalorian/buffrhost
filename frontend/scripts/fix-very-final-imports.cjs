...
const findFiles = () => {
  try {
    const result = execSync(
      'grep -r "@/components/ui/\\(table\\|switch\\|progress\\)" --include="*.tsx" --include="*.ts" .',
      {
...
const updateFile = (filePath) => {
  try {
    ...
    const replacements = [
      { from: '@/components/ui/table', to: 'BuffrTable' },
      { from: '@/components/ui/switch', to: 'BuffrSwitch' },
      { from: '@/components/ui/progress', to: 'BuffrProgress' },
    ];
