...
const findFiles = () => {
  try {
    const result = execSync(
      'grep -r "@/components/ui/\\(daisy-alert\\|badge\\|alert\\)" --include="*.tsx" --include="*.ts" .',
      {
...
const updateFile = (filePath) => {
  try {
    ...
    const replacements = [
      { from: '@/components/ui/daisy-alert', to: 'BuffrAlert' },
      { from: '@/components/ui/badge', to: 'BuffrBadge' },
      { from: '@/components/ui/alert', to: 'BuffrAlert' },
    ];
