...
async function getCompleteDatabaseInfo() {
  try {
    ...
    for (const table of tablesResult.rows) {
      console.log(`\n${'='.repeat(80)}`);
      console.log(`📋 ${table.table_type.toUpperCase()}: ${table.table_name} (${table.column_count} columns)`);
        ...
        if (sampleResult.rows.length > 0) {
          ...
          sampleResult.rows.forEach((row, index) => {
            ...
            Object.entries(row).forEach(([key, value]) => {
              const displayValue = typeof value === 'object' ? JSON.stringify(value).substring(0, 50) + (JSON.stringify(value).length > 50 ? '...' : '') : String(value).substring(0, 50);
    ...
    const statsQueries = [
      ...
      { name: 'Total Views', query: "SELECT COUNT(*) as count FROM information_schema.views WHERE table_schema = 'public'" },
      { name: 'Total Functions', query: "SELECT COUNT(*) as count FROM information_schema.routines WHERE routine_schema = 'public'" },
      { name: 'Total Sequences', query: "SELECT COUNT(*) as count FROM information_schema.sequences WHERE sequence_schema = 'public'" }
    ];
