...
test.describe("Booking Flow", () => {
  ...
  test("completes successful booking flow", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="booking-number"]')).toContainText(
      "BK-2024-001",
    );
  });
  ...
  test("validates required fields", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="guest-name-error"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="guest-email-error"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="check-in-date-error"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="check-out-date-error"]'),
    ).toBeVisible();
  });
  ...
  test("validates check-out date is after check-in date", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="check-out-date-error"]'),
    ).toContainText("Check-out date must be after check-in date");
  });
  ...
  test("validates number of guests", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="guests-error"]')).toContainText(
  ...
  test("shows room availability", async ({ page }) => {
    ...
    // Check room details
    await expect(page.locator('[data-testid="room-name-1"]')).toContainText(
      "Standard Room",
    );
    await expect(page.locator('[data-testid="room-price-1"]')).toContainText(
      "N$500",
    );
  });
