...
describe('HR Components and Pages', () => {
  ...
  test('PayrollRecordForm renders correctly', () => {
    const handleSubmit = jest.fn();
    render(<PayrollRecordForm employeeId="test-employee-id" onSubmit={handleSubmit} />);
  ...
  test('TaxDetailForm renders correctly', () => {
    const handleSubmit = jest.fn();
    render(<TaxDetailForm employeeId="test-employee-id" onSubmit={handleSubmit} />);
  ...
  test('BenefitEnrollmentForm renders correctly', () => {
    const handleSubmit = jest.fn();
    render(<BenefitEnrollmentForm employeeId="test-employee-id" onSubmit={handleSubmit} />);
  ...
  test('PayrollPage renders correctly', () => {
    render(<PayrollPage />);
    expect(screen.getByRole('heading', { name: /Payroll Management/i })).toBeInTheDocument();
  });

  test('TaxPage renders correctly', () => {
    render(<TaxPage />);
    expect(screen.getByRole('heading', { name: /Tax Management/i })).toBeInTheDocument();
  });

  test('BenefitsPage renders correctly', () => {
    render(<BenefitsPage />);
    expect(screen.getByRole('heading', { name: /Benefit Management/i })).toBeInTheDocument();
  });
});
