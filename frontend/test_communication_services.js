...
const {
  BuffrCommunicationService,
} = require('./lib/services/communication/BuffrCommunicationService');
...
async function testCommunicationServices() {
  ...
  try {
    ...
    const calendarAuth =
      await commService.checkServiceAuthorization('google_calendar');
    const whatsappAuth =
      await commService.checkServiceAuthorization('twilio_whatsapp');
  ...
  try {
    ...
    if (emailResult.success) {
      ...
      console.log(
        `   ⚠️ Email sending failed (expected without auth): ${emailResult.error}`
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (calendarResult.success) {
      ...
      console.log(
        `   ⚠️ Calendar booking failed (expected without auth): ${calendarResult.error}`
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (whatsappResult.success) {
      ...
      console.log(
        `   ⚠️ WhatsApp sending failed (expected without auth): ${whatsappResult.error}`
      );
    }
  } catch (error) {
