...
from schemas.financial_settings import (FinancialTransactionResponse,
                                        ...
                                        ServiceRateCreate, ServiceRateResponse,
                                        ServiceRateUpdate)
...
@router.post(
    "/settings/{property_id}", response_model=PropertyFinancialSettingsResponse
)
async def create_financial_settings(
    ...
    try:
        settings = await financial_service.create_or_update_financial_settings(
            property_id, settings_data.dict()
        )
        return settings
    except Exception as e:
        raise HTTPException(
...
async def update_financial_settings(
    ...
    try:
        settings = await financial_service.create_or_update_financial_settings(
            property_id, settings_data.dict(exclude_unset=True)
        )
        return settings
    except Exception as e:
        raise HTTPException(
...
@router.post(
    "/bank-accounts/{property_id}", response_model=PropertyFinancialSettingsResponse
)
...
@router.put(
    "/service-rates/{property_id}/{service_name}", response_model=ServiceRateResponse
)
...
# endpoints
@router.post(
    "/quotations/{property_id}", response_model=PropertyFinancialSettingsResponse
)
...
@router.get(
    "/quotations/{quotation_id}", response_model=PropertyFinancialSettingsResponse
)
...
@router.get(
    "/quotations/property/{property_id}",
    response_model=List[PropertyFinancialSettingsResponse],
)
...
# endpoints
@router.post(
    "/invoices/from-quotation/{quotation_id}",
    response_model=PropertyFinancialSettingsResponse,
)
...
async def record_invoice_payment(
    ...
    try:
        invoice = await financial_service.update_invoice_payment(
            invoice_id, payment_amount, payment_method, payment_reference
        )
...
async def create_receipt(
    ...
    try:
        receipt = await financial_service.create_receipt(property_id, receipt_data)
        return receipt
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_tax_rates(
    year: int, financial_service: FinancialService = Depends(get_financial_service)
):
