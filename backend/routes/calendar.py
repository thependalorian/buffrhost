...
async def get_room_availability(
    ...
    available_rooms = await service.check_room_availability(
...
@router.put(
    "/properties/{property_id}/calendar/rooms/reservations/{reservation_id}/status"
)
async def update_room_reservation_status(
    ...
    service = CalendarService(db)
    reservation = await service.update_room_reservation_status(
...
async def get_service_availability(
    ...
    service_type: str = Query(
        ..., description="Type of service (e.g., spa, conference)"
    ),
    ...
    service_id: Optional[int] = Query(
        None, description="Filter by specific service instance ID"
    ),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> Dict[str, Any]:
    ...
    service = CalendarService(db)
    available_services = await service.check_service_availability(
        property_id, service_type, service_id, booking_date, start_time, end_time
    )
...
async def update_service_booking_status(
    ...
    if not booking:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Service booking not found"
        )
    return {
        "success": True,
...
async def get_combined_schedule(
    ...
    target_date: date = Query(
        ..., description="Target date for the schedule (YYYY-MM-DD)"
    ),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> Dict[str, Any]:
