...
class EtunaDemoAI:
    ...
    def _load_website_knowledge(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pricing": {"currency": "NAD", "tax_rate": 0.15, "service_charge": 0.10},
            "booking_policies": {
    ...
    async def process_message(self, request: EtunaChatRequest) -> EtunaChatResponse:
        ...
        try:
            ...
            processed_response = self._process_response_for_demo(
...
"""
    ...
    def _process_response_for_demo(
        ...
        if (
            "quote" in original_message.lower()
            or "quotation" in original_message.lower()
        ):
            suggestions = [
            ...
            "book" in original_message.lower()
            or "reservation" in original_message.lower()
        ):
            suggestions = [
                "Select room type",
                "Choose dates",
            ...
            "sign up" in original_message.lower()
            or "register" in original_message.lower()
        ):
            suggestions = [
                "Create account",
                "Choose plan",
    ...
    def _format_room_types(self, room_types: list) -> str:
        ...
        for room in room_types:
            formatted.append(
                f"- {room['name']}: NAD {room['price']}/night (sleeps {room['capacity']})"
            )
        return "\n".join(formatted)
    ...
    def _format_tours_info(self, tours: list) -> str:
        ...
        for tour in tours:
            formatted.append(
                f"- {tour['name']}: {tour['duration']} - NAD {tour['price']}"
            )
        return "\n".join(formatted)
...
async def chat_with_etuna_ai(request: EtunaChatRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process message with AI assistant"
        )
