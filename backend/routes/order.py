...
async def create_order(
    ...
    if (
        ...
        and current_user.profile.property_id != property_id
    ):
        raise HTTPException(
    ...
    if order_data.customer_id:
        customer_result = await db.execute(
            select(Profile).where(Profile.id == order_data.customer_id)
        )
    ...
    for item_data in order_data.order_items:
        ...
        for option_data in item_data.selected_options:
            option_value_result = await db.execute(
                select(MenuOptionValue).where(
                    MenuOptionValue.option_value_id == option_data.option_value_id
                )
            )
