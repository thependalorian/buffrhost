...
from models.cms import ContentType, ContentStatus
...
async def add_content_to_collection(
    ...
    cms_service = CMSService(db)
    success = cms_service.add_content_to_collection(collection_id, content_id, display_order)
    
    if not success:
        raise HTTPException(status_code=400, detail="Failed to add content to collection")
