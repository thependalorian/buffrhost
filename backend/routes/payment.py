...
from schemas.payment import (AdumoCallbackData, AdumoPaymentRequest,
                             AdumoPaymentResponse, PaymentStatusUpdate,
...
def get_adumo_service() -> AdumoPaymentService:
    ...
    if adumo_service is None:
        ...
        if not all(
            [
                settings.ADUMO_MERCHANT_ID,
                settings.ADUMO_APPLICATION_ID,
                settings.ADUMO_JWT_SECRET,
            ]
        ):
            raise HTTPException(
...
@router.post(
    "/orders/{order_id}/payments/adumo/initialize", response_model=AdumoPaymentResponse
)
...
async def adumo_payment_callback(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not payment_transaction:
            logger.error(
                f"Payment transaction not found for reference: {callback_data._MERCHANTREFERENCE}"
            )
            raise HTTPException(
        ...
        if not success:
            logger.error(
                f"Failed to update payment status for transaction {payment_transaction.transaction_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.info(
            f"Processed Adumo callback for transaction {payment_transaction.transaction_id}"
        )
...
async def adumo_payment_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not payment_transaction:
            logger.error(
                f"Payment transaction not found for webhook reference: {merchant_reference}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_F,
                detail="Payment transaction not found",
        ...
        logger.info(
            f"Received Adumo webhook for transaction {payment_transaction.transaction_id}"
        )
