...
from auth.dependencies import \
    get_current_admin_user  # Assuming only admins can manage banking/financial
...
async def get_banking_financial_service(
    db: AsyncSession = Depends(get_db),
) -> BankingFinancialService:
    return BankingFinancialService(db)


# --- Bank Account Endpoints ---
@router.post(
    "/bank-accounts", response_model=Response, status_code=status.HTTP_201_CREATED
)
...
async def update_transaction(
    ...
    transaction = await banking_service.update_transaction(
...
async def get_disbursement(
    ...
    if not disbursement:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Disbursement not found"
        )
    return disbursement
...
async def update_disbursement(
    ...
    disbursement = await banking_service.update_disbursement(
        disbursement_id, disbursement_data
    )
    if not disbursement:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Disbursement not found"
...
@router.delete(
    "/disbursements/{disbursement_id}", status_code=status.HTTP_204_NO_CONTENT
)
