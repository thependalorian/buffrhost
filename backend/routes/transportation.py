...
from schemas.transportation import TransportationService, TransportationBooking, TransportationBookingCreate, TransportationVehicle, TransportationRoute

router = APIRouter()

@router.get("/{property_id}/transportation/services", response_model=List[TransportationService])
async def get_transportation_services(
    ...
    return await transportation_service.get_transportation_services(property_id=property_id)

@router.get("/{property_id}/transportation/bookings", response_model=List[TransportationBooking])
async def get_transportation_bookings(
    ...
    transportation_service = TransportationServiceClass(db)
    return await transportation_service.get_transportation_bookings(property_id=property_id, start_date=start_date, end_date=end_date)

@router.post("/{property_id}/transportation/bookings", response_model=TransportationBooking)
async def create_transportation_booking(
    ...
    transportation_service = TransportationServiceClass(db)
    try:
        return await transportation_service.create_transportation_booking(property_id=property_id, booking_data=booking_data)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))

@router.get("/{property_id}/transportation/vehicles", response_model=List[TransportationVehicle])
...
@router.get("/{property_id}/transportation/routes", response_model=List[TransportationRoute])
