...
from schemas.spa import SpaService, SpaAppointment, SpaTherapist, SpaAppointmentCreate
...
async def get_spa_appointments(
    ...
    spa_service = SpaServiceClass(db)
    return await spa_service.get_spa_appointments(property_id=property_id, start_date=start_date, end_date=end_date)
...
async def create_spa_appointment(
    ...
    spa_service = SpaServiceClass(db)
    try:
        return await spa_service.create_spa_appointment(property_id=property_id, appointment_data=appointment_data)
