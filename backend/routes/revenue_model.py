...
from auth.dependencies import \
    get_current_admin_user  # Assuming only admins can manage revenue models
from database import get_db
from models.user import User
from schemas.revenue_model import (CommissionStructureCreate,
                                   CommissionStructureResponse,
                                   CommissionStructureUpdate, Create, Response,
                                   ServiceFeeCreate, ServiceFeeResponse,
                                   ServiceFeeUpdate, SubscriptionCreate,
...
async def get_revenue_model_service(
...
async def update_subscription(
    ...
    subscription = await revenue_service.update_subscription(
...
@router.post(
    "/service-fees",
    response_model=ServiceFeeResponse,
    status_code=status.HTTP_201_CREATED,
)
...
async def get_service_fee(
    ...
    if not fee:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Service fee not found"
        )
    return fee
...
@router.post(
    ...
    response_model=CommissionStructureResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.get(
    "/commission-structures/{commission_id}", response_model=CommissionStructureResponse
)
...
async def update_commission_structure(
    ...
    commission = await revenue_service.update_commission_structure(
        commission_id, commission_data
    )
    if not commission:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@router.delete(
    "/commission-structures/{commission_id}", status_code=status.HTTP_204_NO_CONTENT
)
