...
from schemas.hr_payroll import (
    ...
    TaxDetailCreate, TaxDetailUpdate, TaxDetailResponse,
...
async def get_hr_payroll_service(db: AsyncSession = Depends(get_db)) -> HRPayrollService:
...
@router.get("/employees/{employee_id}/payroll-records", response_model=List[PayrollRecordResponse])
...
@router.post("/tax-details", response_model=TaxDetailResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/employees/{employee_id}/tax-details", response_model=List[TaxDetailResponse])
...
async def get_tax_detail(
    ...
    if not detail:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tax detail not found")
    return detail
...
@router.post("/benefit-enrollments", response_model=BenefitEnrollmentResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/employees/{employee_id}/benefit-enrollments", response_model=List[BenefitEnrollmentResponse])
...
@router.get("/benefit-enrollments/{benefit_id}", response_model=BenefitEnrollmentResponse)
async def get_benefit_enrollment(
    ...
    if not enrollment:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Benefit enrollment not found")
    return enrollment

@router.put("/benefit-enrollments/{benefit_id}", response_model=BenefitEnrollmentResponse)
...
@router.delete("/benefit-enrollments/{benefit_id}", status_code=status.HTTP_204_NO_CONTENT)
