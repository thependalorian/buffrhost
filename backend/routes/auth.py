...
@router.post(
    "/forgot-password", dependencies=[Depends(RateLimiter(times=2, seconds=120))]
)
    ...
    if user:
        password_reset_token = create_access_token(
            ...
            expires_delta=timedelta(
                minutes=getattr(settings, "PASSWORD_RESET_TOKEN_EXPIRE_MINUTES", 15)
            ),
        )
...
@router.post(
    "/reset-password", dependencies=[Depends(RateLimiter(times=2, seconds=120))]
)
...
async def require_permission(
    permission: Permission, user: User = Depends(get_current_user)
):
...
async def require_property_access(
    property_id: int, user: User = Depends(get_current_user)
):
