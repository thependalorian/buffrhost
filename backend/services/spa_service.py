...
from models.staff import StaffProfile, StaffPosition
from schemas.spa import SpaService as SpaServiceSchema, SpaAppointment as SpaAppointmentSchema, SpaTherapist as SpaTherapistSchema, SpaAppointmentCreate
...
class SpaServiceClass:
    ...
    async def get_spa_services(
        ...
        query = select(SpaService).where(SpaService.property_id == property_id).offset(skip).limit(limit)
    ...
    async def get_spa_therapists(
        ...
        query = select(StaffProfile).join(StaffPosition).where(
    ...
    async def create_spa_appointment(
        ...
        booking = ServiceBooking(
            ...
            end_time=(appointment_data.appointment_time + timedelta(minutes=service.duration_minutes)).time(),
