...
class PaymentStatusService:
        ...
        try:
            ...
            if additional_data:
                if "adumo_transaction_index" in additional_data:
                    payment_transaction.adumo_transaction_index = additional_data[
                        "adumo_transaction_index"
                    ]
                if "adumo_result_code" in additional_data:
                    payment_transaction.adumo_result_code = additional_data[
                        "adumo_result_code"
                    ]
                ...
                if "payment_method" in additional_data:
                    payment_transaction.payment_method = additional_data[
                        "payment_method"
                    ]
                if "error_message" in additional_data:
                    payment_transaction.adumo_error_message = additional_data[
                        "error_message"
                    ]

            # Update related order
            await self._update_order_status(
                db, payment_transaction.order_id, is_successful
            )

            # Send notifications
            await self._send_payment_notifications(
                db, payment_transaction, is_successful
            )

            await db.commit()
            logger.info(
                f"Updated payment status for transaction {transaction_id}: {status}"
            )
            return True
    ...
    async def _update_order_status(
        self, db: AsyncSession, order_id: UUID, payment_successful: bool
    ) -> None:
        ...
        try:
            ...
            if data.get("customer_phone"):
                ...
                await self.notification_service.send_sms(
                    to_phone=data["customer_phone"], message=message
                )
    ...
    async def get_payment_analytics(
        ...
        try:
            ...
            average_amount = (
                (total_amount / successful_transactions)
    ...
    async def process_webhook_notification(
        ...
        try:
            ...
            if not payment_transaction:
                logger.error(
                    f"Payment transaction not found for webhook: {merchant_reference}"
                )
                return False
            ...
            logger.info(
                f"Processed webhook for transaction {payment_transaction.transaction_id}"
            )
            return True
