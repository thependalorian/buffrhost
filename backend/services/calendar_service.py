...
class CalendarService:
    ...
    async def check_room_availability(
        ...
        for room in all_rooms:
            ...
            overlapping_reservations = (
                (await self.db.execute(overlapping_reservations_query)).scalars().all()
            )
        ...
        for service_instance in all_services:
            ...
            overlapping_bookings_query = select(ServiceBooking).where(
                and_(
                    ...
                    == service_instance.service_id,  # Assuming service_id is consistent
                    ServiceBooking.booking_date == booking_date,
                    ServiceBooking.status.in_(["confirmed"]),
                    or_(
                        and_(
                            ServiceBooking.start_time < end_time,
                            ServiceBooking.end_time > start_time,
                        ),
                        and_(
                            ServiceBooking.start_time >= start_time,
                            ServiceBooking.start_time < end_time,
                        ),
                        and_(
                            ServiceBooking.end_time > start_time,
                            ServiceBooking.end_time <= end_time,
                        ),
                    ),
                )
            )
            overlapping_bookings = (
                (await self.db.execute(overlapping_bookings_query)).scalars().all()
            )
    ...
    async def get_daily_schedule(
        ...
        room_reservations = (
            (await self.db.execute(room_reservations_query)).scalars().all()
        )
        ...
        service_bookings = (
            (await self.db.execute(service_bookings_query)).scalars().all()
        )
        ...
        reservation = (
            await self.db.execute(
                select(RoomReservation).where(
                    RoomReservation.reservation_id == reservation_id
                )
            )
        ).scalar_one_or_none()
        if reservation:
    ...
    async def update_service_booking_status(
        ...
        booking = (
            await self.db.execute(
                select(ServiceBooking).where(ServiceBooking.booking_id == booking_id)
            )
        ).scalar_one_or_none()
        if booking:
