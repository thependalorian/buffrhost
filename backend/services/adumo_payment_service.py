...
class AdumoPaymentService:
    ...
    def validate_response_token(
        self, response_token: str, expected_amount: Decimal, expected_mref: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Successfully validated response token for order {expected_mref}"
            )
            return decoded
    ...
    def create_payment_form_data(
        ...
        try:
            ...
            # Generate JWT token
            notification_url = (
                f"{self.base_url.replace('apiv3', 'webhooks')}/buffr/notifications"
            )
            token = self.generate_jwt_token(
                amount, merchant_reference, notification_url
            )
