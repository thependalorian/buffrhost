...
from models.inventory import (InventoryItem, MenuItemRawMaterial,
                              UnitOfMeasurement)
...
class InventoryService:
    ...
    async def get_inventory_items(
        ...
        query = select(InventoryItem).where(InventoryItem.property_id == property_id)
    ...
    async def get_low_stock_items(self, property_id: int) -> List[Dict[str, Any]]:
    ...
    async def update_stock(
        self, inventory_id: int, new_stock: float, property_id: int
    ) -> Dict[str, Any]:
    ...
    async def get_menu_item_ingredients(
        self, menu_item_id: int, property_id: int
    ) -> List[Dict[str, Any]]:
        """Get ingredients required for a menu item."""
        query = (
            select(MenuItemRawMaterial, InventoryItem)
            .join(
                InventoryItem,
                MenuItemRawMaterial.inventory_id == InventoryItem.inventory_id,
            )
            .where(
                and_(
        ...
        return [
            {
                ...
                "sufficient_stock": ingredient.InventoryItem.current_stock
                >= ingredient.MenuItemRawMaterial.quantity,
            }
            for ingredient in ingredients
        ]

    async def get_inventory_usage_analytics(
        self, property_id: int, days: int = 30
    ) -> Dict[str, Any]:
        ...
        low_stock_items = len(
            [item for item in items if item.current_stock <= item.minimum_stock]
        )
        ...
        return {
            ...
            "low_stock_percentage": (low_stock_items / total_items * 100)
            if total_items > 0
            else 0,
            "out_of_stock_percentage": (out_of_stock_items / total_items * 100)
            if total_items > 0
            else 0,
        }
