...
class RAGSystem:
    ...
    async def query(
        ...
        try:
            ...
            return RAGResponse(
                ...
                sources=[],
                related_questions=[],
                suggested_actions=["Contact support team", "Try rephrasing your question"]
            )
    ...
    async def _classify_query_type(self, question: str) -> QueryType:
        ...
        if any(word in question_lower for word in ["revenue", "sales", "profit", "analytics", "performance", "metrics", "report"]):
            return QueryType.BUSINESS_ANALYTICS
        
        # Customer support keywords
        elif any(word in question_lower for word in ["customer", "guest", "support", "help", "problem", "issue"]):
            return QueryType.CUSTOMER_SUPPORT
        
        # Operational guidance keywords
        elif any(word in question_lower for word in ["how to", "process", "procedure", "operation", "workflow"]):
            return QueryType.OPERATIONAL_GUIDANCE
        
        # Policy questions
        elif any(word in question_lower for word in ["policy", "rule", "regulation", "compliance", "terms"]):
            return QueryType.POLICY_QUESTION
        
        # Hospitality knowledge
        elif any(word in question_lower for word in ["hospitality", "hotel", "restaurant", "service", "amenity"]):
            return QueryType.HOSPITALITY_KNOWLEDGE
        
        else:
            return QueryType.GENERAL_QUESTION
    
    async def _retrieve_context(self, question: str, property_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"sources": [], "context_text": "", "property_context": {}, "total_sources": 0}
    
    async def _get_property_context(self, property_id: int) -> Dict[str, Any]:
        """Get property-specific context for queries"""
        try:
            # Get property information
            query = select(HospitalityProperty).where(HospitalityProperty.id == property_id)
    ...
    async def _get_property_metrics(self, property_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            reservation_query = select(RoomReservation).where(
                RoomReservation.property_id == property_id
            ).order_by(RoomReservation.created_at.desc()).limit(100)
            ...
            occupancy_rate = len(reservations) / 100 if reservations else 0  # Assuming 100 rooms
    ...
    async def _recreate_index_with_updates(self, document_id: str, updated_content: str):
