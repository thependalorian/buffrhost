...
class RecommendationEngine:
    ...
    def __init__(self, db_session: AsyncSession):
        ...
        self.service_similarity = {
            ...
            "transportation": {"conference": 0.4, "recreation": 0.3, "specialized": 0.2},
    ...
    async def get_recommendations(self, request: RecommendationRequest) -> RecommendationResponse:
        ...
        try:
            ...
            return RecommendationResponse(
                ...
                algorithm_used=[rec.recommendation_type for rec in final_recommendations]
            )
    ...
    async def _analyze_group_size_preferences(self, customer_id: int) -> Dict[str, int]:
        ...
        try:
            service_history = await self._get_service_history(customer_id)
            if not service_history:
                return {"preferred_group_size": 2, "max_group_size": 4, "solo_travel": 0.3, "couple_travel": 0.5, "group_travel": 0.2}

            group_sizes = [service.get('number_of_guests', 1) for service in service_history if service.get('number_of_guests')]
            if not group_sizes:
                return {"preferred_group_size": 2, "max_group_size": 4, "solo_travel": 0.3, "couple_travel": 0.5, "group_travel": 0.2}
    ...
    async def _get_available_services(self, property_id: int) -> Dict[str, List[Any]]:
        ...
        try:
            ...
            conference_query = select(ConferenceRoom).where(ConferenceRoom.property_id == property_id)
            ...
            transport_query = select(TransportationService).where(TransportationService.property_id == property_id)
            ...
            recreation_query = select(RecreationService).where(RecreationService.property_id == property_id)
            ...
            specialized_query = select(SpecializedService).where(SpecializedService.property_id == property_id)
    ...
    async def _get_collaborative_recommendations(
        ...
        try:
            ...
            for similar_guest_id in similar_guests[:5]:  # Top 5 similar guests
                similar_guest_history = await self._get_service_history(similar_guest_id)
                ...
                for service in similar_guest_history:
                    if service["service_type"] in available_services:
                        ...
                        rec = await self._create_recommendation_from_service(
                            service, available_services, RecommendationType.COLLABORATIVE,
                ...
                for similar_type, similarity_score in similar_types.items():
                    if similarity_score > 0.5:  # Only high similarity
                        ...
                        for service in services[:3]:  # Top 3 services
                            rec = await self._create_recommendation_from_service(
                                {"service_type": similar_type, "service_id": str(service.id)},
                                available_services, RecommendationType.CONTENT_BASED,
            ...
            for service_type in preferred_service_types:
                ...
                for service in services[:2]:  # Top 2 from each preferred type
                    rec = await self._create_recommendation_from_service(
                        ...
                        available_services, RecommendationType.PERSONALIZED,
        ...
        try:
            ...
            for service_type, services in available_services.items():
                ...
                for service in popular_services:
                    rec = await self._create_recommendation_from_service(
                        {"service_type": service_type, "service_id": str(service.id)},
                        available_services, RecommendationType.POPULAR,
    ...
    async def _get_seasonal_recommendations(
        ...
        try:
            ...
            preferred_types = seasonal_preferences.get(current_month, ["restaurant", "hotel"])
            ...
            for service_type in preferred_types:
                ...
                for service in services[:1]:  # Top 1 seasonal service
                    rec = await self._create_recommendation_from_service(
                        {"service_type": service_type, "service_id": str(service.id)},
                        available_services, RecommendationType.SEASONAL,
    ...
    async def update_recommendation_feedback(
        ...
        try:
            ...
            logger.info(f"Updated recommendation feedback for customer {customer_id}, service {service_id}: {feedback} (rating: {rating})")
    ...
    async def _train_collaborative_filtering_model(self, user_item_matrix: np.ndarray):
        ...
        try:
            ...
            logger.info(f"Collaborative filtering model trained with {np.sum(active_users)} active users")
            
        except Exception as e:
            logger.error(f"Error training collaborative filtering model: {e}")
    
    async def _train_content_based_model(self, service_features: np.ndarray, ratings: np.ndarray):
        ...
        try:
            ...
            logger.info(f"Content-based model trained with MSE: {mse:.3f}, R²: {r2_score:.3f}")
    ...
    async def _get_ml_based_recommendations(self, guest_profile: GuestProfile, 
            ...
            if self.content_model is not None:
                for service in available_services:
                    ...
                    if service_features is not None:
                        ...
                        # Predict rating
                        predicted_rating = self.content_model.predict(features_scaled)[0]
        ...
        try:
            ...
            for customer_id, menu_item_id, quantity, total_amount in interactions:
                if customer_id in customer_to_idx and menu_item_id in item_to_idx:
                    ...
                    interaction_strength = min(quantity * (total_amount / 100), 5.0)  # Normalize to 0-5 scale
    ...
    async def _predict_rating_from_similar_users(self, service: Dict, similar_users: np.ndarray, 
        ...
        try:
            ...
            for user_idx in similar_users:
                if user_idx < user_item_matrix.shape[0] and item_idx < user_item_matrix.shape[1]:
    ...
    async def _extract_service_features(self, service: Dict) -> Optional[np.ndarray]:
        ...
        try:
            ...
            price_query = select(func.avg(Menu.base_price), func.stddev(Menu.base_price))
            ...
            ).join(OrderItem).where(OrderItem.menu_item_id == service_id)
            ...
            rating_confidence = min(order_count or 0 / 10, 1.0)  # Max confidence at 10+ orders
            ...
            recent_popularity_query = select(func.count(OrderItem.order_item_id)).join(Order).where(
                OrderItem.menu_item_id == service_id,
                Order.created_at >= recent_date
            )
            recent_popularity_result = await self.db_session.execute(recent_popularity_query)
            ...
            customer_spending_query = select(func.avg(Order.total_amount)).join(OrderItem).where(
                OrderItem.menu_item_id == service_id
            )
            customer_spending_result = await self.db_session.execute(customer_spending_query)
    ...
    async def _cluster_guests(self, guest_profiles: List[GuestProfile]) -> Dict[int, int]:
    ...
    async def _get_cluster_based_recommendations(self, guest_profile: GuestProfile, 
                                               available_services: List[Dict],
                                               guest_clusters: Dict[int, int]) -> List[Recommendation]:
        ...
        try:
            ...
            cluster_guests = [gid for gid, cid in guest_clusters.items() if cid == cluster_id]
    ...
    async def _calculate_cluster_popularity(self, service: Dict, cluster_guests: List[int]) -> float:
        ...
        try:
            ...
            usage_query = select(func.count(func.distinct(Order.customer_id))).join(OrderItem).where(
                OrderItem.menu_item_id == service_id,
            ...
            spending_query = select(func.avg(Order.total_amount)).join(OrderItem).where(
                OrderItem.menu_item_id == service_id,
                Order.customer_id.in_(cluster_guests)
            )
        ...
        try:
            ...
            for menu_id, name, base_price, category, description in menu_items:
                ...
                order_query = select(
                    ...
                    Order.total_amount,
                    Order.created_at
                ).join(OrderItem).where(OrderItem.menu_item_id == menu_id)
                ...
                for customer_id, quantity, total_amount, created_at in orders:
                    user_item_data.append((customer_id, menu_id, quantity, total_amount))
            
            return np.array(service_features), np.array(ratings), np.array(user_item_data)
    ...
    async def train_models(self):
        ...
        try:
            ...
            service_features, ratings, user_item_data = await self._collect_training_data()
            ...
            user_item_matrix = await self._build_user_item_matrix(1)  # Pass any customer_id
    ...
    async def _evaluate_models(self, X_test: np.ndarray, y_test: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if self.content_model is not None:
                ...
                cv_scores = cross_val_score(self.content_model, X_test_scaled, y_test, cv=5, scoring='r2')
            ...
            if self.ensemble_model is not None:
                ...
                cv_scores_gb = cross_val_score(self.ensemble_model, X_test_scaled, y_test, cv=5, scoring='r2')
    ...
    async def _feature_importance_analysis(self) -> Dict[str, List[Tuple[str, float]]]:
        ...
        try:
            ...
            feature_names = [
                'base_price', 'normalized_price', 'normalized_rating', 'rating_confidence',
                'rating_variance', 'popularity', 'popularity_percentile', 'month',
                'hour', 'weekday', 'duration_minutes', 'capacity', 'price_tier',
                'recent_popularity', 'avg_customer_spending', 'value_ratio', 'popularity_ratio'
            ]
            ...
            if self.content_model is not None and hasattr(self.content_model, 'feature_importances_'):
                rf_importance = list(zip(feature_names, self.content_model.feature_importances_))
                rf_importance.sort(key=lambda x: x[1], reverse=True)
                feature_importance['random_forest'] = rf_importance[:10]  # Top 10 features
            
            # Gradient Boosting feature importance
            if self.ensemble_model is not None and hasattr(self.ensemble_model, 'feature_importances_'):
                gb_importance = list(zip(feature_names, self.ensemble_model.feature_importances_))
                gb_importance.sort(key=lambda x: x[1], reverse=True)
                feature_importance['gradient_boosting'] = gb_importance[:10]  # Top 10 features
