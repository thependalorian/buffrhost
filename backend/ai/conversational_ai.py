...
class ConversationalAI:
    ...
    def __init__(self, db_session: AsyncSession, openai_api_key: str, database_url: str):
    ...
    def _initialize_arcade_tools(self):
        ...
        try:
            ...
            self.tool_manager = ToolManager(api_key=self.arcade_service.client.api_key if self.arcade_service.client else None)
    ...
    def _create_enhanced_conversation_workflow(self) -> StateGraph:
        ...
        workflow.add_node("tools", self.tool_node if self.tool_node else self._dummy_tool_node)
        ...
        workflow.add_conditional_edges("classify_message", self._should_use_rag, ["rag_query", "agent"])
        workflow.add_conditional_edges("agent", self._should_continue, ["authorization", "tools", END])
    ...
    async def _should_use_rag(self, state: MessagesState) -> str:
        ...
        if message_type in [MessageType.GENERAL_QUESTION, MessageType.SERVICE_REQUEST, MessageType.BOOKING_INQUIRY]:
            return "rag_query"
        else:
            return "agent"
    
    async def _call_agent(self, state: MessagesState, writer=None, config: RunnableConfig = None, *, store: BaseStore = None):
        """Call the main agent with memory and tool integration."""
        messages = state["messages"]
        
        # Get user_id from config
        user_id = config.get("configurable", {}).get("user_id", "anonymous").replace(".", "")
        ...
        if last_user_message and "remember" in str(last_user_message.content).lower() and store:
            try:
    ...
    async def _authorize(self, state: MessagesState, config: RunnableConfig = None, writer=None, *, store: BaseStore = None):
        ...
        for tool_call in state["messages"][-1].tool_calls:
            ...
            try:
                ...
                if writer:
                    writer(f"❌ Authorization failed for {tool_name}. Please try again.\n")
    ...
    async def _rag_query(self, state: MessagesState) -> MessagesState:
        ...
        try:
            ...
            property_id = state.get('context', {}).get('property_id', 1)  # Default to property 1
            ...
            response = AIMessage(content="I apologize, but I'm having trouble accessing our knowledge base right now. Please contact our staff for assistance.")
            return {"messages": [response]}
    ...
    async def process_message(
        ...
        try:
            ...
            response_content = final_response.content if final_response else "I apologize, but I couldn't process your request."
