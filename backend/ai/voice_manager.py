...
class EnhancedVoiceManager:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stt_models": {"openai": {"model": "whisper-1"}},
            "audio_settings": {"sample_rate": 22050, "channels": 1, "format": "wav"},
        }
    ...
    async def load_tts_model(self, model_type: TTSModel) -> bool:
        ...
        try:
            ...
            if model_type == TTSModel.KOKORO_82M and HAS_LOCAL_MODELS:
                ...
                logger.warning(
                    f"Model {model_type} not available, falling back to OpenAI"
                )
                return True
    ...
    async def _load_kokoro_model(self) -> bool:
        ...
        try:
            ...
            self.tts_models[TTSModel.KOKORO_82M] = ort.InferenceSession(
    ...
    async def _load_xtts_model(self) -> bool:
        ...
        try:
            ...
            self.tts_models[TTSModel.XTTS_V2] = TTS(
    ...
    async def _openai_tts(self, text: str, settings: VoiceSettings) -> bytes:
        ...
        try:
            response = await self.openai_client.audio.speech.create(
                model="tts-1", voice=settings.voice, input=text, speed=settings.speed
            )
    ...
    async def _whisper_openai_stt(
        self, audio_data: bytes, settings: VoiceSettings
    ) -> str:
    ...
    def get_supported_voices(
        self, model_type: TTSModel, language: str = "en"
    ) -> List[str]:
        ...
        if model_type == TTSModel.KOKORO_82M:
            ...
            return [
                f"en_US-{name}"
                for name in ["amy", "danny", "joe", "kathleen", "lessac", "ryan"]
            ]
    ...
    async def test_voice_capabilities(self) -> Dict[str, Any]:
        ...
        for model in TTSModel:
            try:
                ...
                test_results["tts_models"][model.value] = (
        ...
        for model in STTModel:
            try:
                ...
                test_results["stt_models"][model.value] = (
                    "available" if success else "unavailable"
                )
            except Exception as e:
        ...
        available_tts = sum(
            1 for status in test_results["tts_models"].values() if status == "available"
        )
        available_stt = sum(
            1 for status in test_results["stt_models"].values() if status == "available"
        )
