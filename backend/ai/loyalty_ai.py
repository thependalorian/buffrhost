...
class LoyaltyAI:
    ...
    def __init__(self, db_session: AsyncSession, openai_api_key: str):
        ...
        self.memory = ConversationBufferMemory(
            memory_key="campaign_history", return_messages=True
        )
    ...
    def _get_loyalty_system_prompt(self) -> str:
        ...
        **Campaign Optimization Guidelines:**
        1. **User-Centric**: Design campaigns that provide genuine value to customers
        ...
        **Campaign Types:**
        ...
        - Referral: User acquisition through referrals
        ...
        **User Segments:**
        ...
        - At Risk: Users showing churn signals
    ...
    def _initialize_segment_definitions(self) -> Dict[UserSegment, Dict[str, Any]]:
    ...
    def _initialize_campaign_templates(self) -> Dict[CampaignType, Dict[str, Any]]:
        ...
        return {
            ...
            CampaignType.REFERRAL: {
                ...
                "reward_value": 200,
                "duration": 60,
                "description": "User referral programs",
            },
    ...
    async def analyze_customer_segments(
        ...
        try:
            ...
            for segment_type in UserSegment:
    ...
    async def _analyze_segment(
        self, property_id: int, segment: UserSegment
    ) -> Optional[UserSegmentAnalysis]:
    ...
    async def _get_customers_in_segment(
        self, property_id: int, segment: UserSegment
    ) -> List[User]:
        ...
        try:
            ...
            if segment == UserSegment.HIGH_VALUE:
            ...
            elif segment == UserSegment.FREQUENT:
                ...
                query = select(User).where(
                    and_(
                        User.property_id == property_id,
                        User.last_login <= datetime.now() - timedelta(days=60),
                    )
                )
                result = await self.db.execute(query)
                customers = result.scalars().all()
                ...
                query = select(User).where(
                    and_(
                        User.property_id == property_id,
                        User.last_login <= datetime.now() - timedelta(days=180),
                    )
                )
                result = await self.db.execute(query)
                customers = result.scalars().all()
                ...
                query = (
                    ...
                    .where(
                        and_(
                            User.property_id == property_id,
                            CrossBusinessLoyalty.tier_level.in_(
            ...
            elif segment == UserSegment.BUDGET:
                # Get budget-conscious customers
                query = select(User).where(
                    and_(
                        User.property_id == property_id,
                        User.avg_order_value <= 50,
                    )
                )
                result = await self.db.execute(query)
                customers = result.scalars().all()
    ...
    async def _calculate_avg_spending(self, customers: List[User]) -> float:
    ...
    async def _calculate_avg_frequency(self, customers: List[User]) -> float:
    ...
    async def _calculate_loyalty_score(self, customers: List[User]) -> float:
    ...
    async def _calculate_churn_risk(self, customers: List[User]) -> float:
    ...
    async def _calculate_growth_potential(self, customers: List[User]) -> float:
    ...
    async def _get_recommended_campaigns(
        self, segment: UserSegment
    ) -> List[CampaignType]:
        ...
        try:
            recommendations = {
                ...
                UserSegment.NEW: [CampaignType.WELCOME, CampaignType.CROSS_SELL],
                UserSegment.AT_RISK: [
                    CampaignType.RETENTION,
                    CampaignType.REACTIVATION,
                ],
                ...
                UserSegment.VIP: [CampaignType.UPSELL, CampaignType.EXPERIENCE],
                UserSegment.BUDGET: [CampaignType.DISCOUNT, CampaignType.SEASONAL],
                UserSegment.BUSINESS: [
                    CampaignType.CROSS_SELL,
    ...
    async def _calculate_expected_participation(
        self, segment_analysis: UserSegmentAnalysis, campaign_type: CampaignType
    ) -> float:
    ...
    async def _calculate_expected_roi(
        ...
        try:
            ...
            campaign_cost = (
                segment_analysis.customer_count * expected_participation * reward_value
            )
    ...
    async def _generate_campaign_reasoning(
        ...
        try:
            ...
            reasoning_parts.append(
                f"Targeting {segment_analysis.segment.value} segment with {segment_analysis.customer_count} customers"
            )

            # Campaign rationale
            if campaign_type == CampaignType.WELCOME:
                reasoning_parts.append(
                    "New customers need onboarding to establish loyalty"
                )
            elif campaign_type == CampaignType.RETENTION:
                reasoning_parts.append(
                    "Existing customers need engagement to prevent churn"
                )
                ...
                reasoning_parts.append(
                    "Cross-service promotion increases customer value"
                )
            elif campaign_type == CampaignType.UPSELL:
                reasoning_parts.append(
                    "Premium service promotion increases revenue per customer"
                )

            # Performance expectations
            reasoning_parts.append(
                f"Expected {expected_participation:.1%} participation rate"
            )
            ...
            if segment_analysis.loyalty_score > 0.7:
                reasoning_parts.append(
                    "High loyalty score indicates strong engagement potential"
                )
    ...
    async def _get_customer_total_spending(self, customer_id: int) -> float:
        ...
        try:
            # Get orders
            order_query = select(func.sum(Order.total_amount)).where(
    ...
    async def _get_customer_loyalty_score(self, customer_id: int) -> float:
        ...
        try:
            ...
            query = select(CrossBusinessLoyalty).where(
    ...
    async def _is_customer_at_risk(self, customer_id: int) -> bool:
        ...
        try:
            ...
            if customer.last_login and customer.last_login < datetime.now() - timedelta(
                days=60
            ):
                return True
    ...
    async def get_loyalty_analytics(self, property_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get customer counts
            customer_query = select(func.count(User.id)).where(
                User.property_id == property_id
            )
            ...
            tier_query = (
                ...
                .where(CrossBusinessLoyalty.property_id == property_id)
    ...
    async def _extract_customer_features(self, customer: User) -> np.ndarray:
        ...
        try:
            ...
            features.append(
                (datetime.now() - customer.created_at).days
            )  # User age in days
    ...
    async def _get_avg_spending(self, customer_id: int) -> float:
        ...
        try:
            query = select(func.avg(Order.total_amount)).where(
                Order.customer_id == customer_id
            )
            result = await self.db.execute(query)
    ...
    async def _get_days_since_last_visit(self, customer_id: int) -> int:
        ...
        try:
            query = select(func.max(Order.created_at)).where(
                Order.customer_id == customer_id
            )
            result = await self.db.execute(query)
    ...
    async def _get_visit_consistency(self, customer_id: int) -> float:
        ...
        try:
            ...
            consistency = (
                max(0, 1 - (std_interval / mean_interval)) if mean_interval > 0 else 0
            )
    ...
    async def _get_campaign_response_rate(self, customer_id: int) -> float:
        ...
        try:
            ...
            campaign_query = select(func.count(LoyaltyTransaction.id)).where(
                and_(
                    ...
                    LoyaltyTransaction.transaction_type.in_(
                        ["campaign_bonus", "promotional_points", "referral_bonus"]
                    ),
                )
            )
            ...
            estimated_campaigns = max(
                1, customer_tenure_days // 30
            )  # Assume monthly campaigns

            # Calculate response rate
            response_rate = (
                campaign_responses / estimated_campaigns
                if estimated_campaigns > 0
                else 0.0
            )
    ...
    async def _get_service_preferences(self, customer_id: int) -> List[float]:
        ...
        try:
            preferences = [
                0.0
            ] * 5  # 5 service categories: restaurant, spa, conference, transportation, accommodation

            # Get restaurant usage
            restaurant_query = select(func.count(Order.id)).where(
                Order.customer_id == customer_id
            )
            ...
            for order in orders:
                ...
                if hasattr(order, "payment_method") and order.payment_method:
                    payment_methods[order.payment_method] = (
                        payment_methods.get(order.payment_method, 0) + 1
                    )

            # Calculate features
            payment_success_rate = (
                successful_payments / total_orders if total_orders > 0 else 0.0
            )
            discount_usage_rate = (
                discount_usage / total_orders if total_orders > 0 else 0.0
            )
            late_payment_rate = (
                late_payments / total_orders if total_orders > 0 else 0.0
            )
    ...
    async def train_ml_models(self, property_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            customers_query = select(User).where(User.property_id == property_id)
            ...
            if len(customers) < 10:
                ...
                return {
                    ...
                    "message": "Need at least 10 customers",
                }
            ...
            if len(customer_features) < 10:
                ...
                return {
                    ...
                    "message": "Need at least 10 customers with features",
                }
            ...
            # 1. Train User Segmentation Model
            try:
            ...
            try:
                ...
                for customer_id in customer_ids:
                    days_since_visit = await self._get_days_since_last_visit(
                        customer_id
                    )
                ...
                if len(set(y_churn)) > 1:  # Need both classes
                    ...
                    training_results["churn_prediction"] = {
                        ...
                        "recall": recall,
                        "f1_score": f1,
                        "feature_importance": self.ml_models[
                            "churn_prediction"
    ...
    def _map_clusters_to_segments(
        self, clusters: np.ndarray, customer_ids: List[int]
    ) -> Dict[int, str]:
        ...
        for cluster_id in set(clusters):
            cluster_customers = [
                customer_ids[i] for i, c in enumerate(clusters) if c == cluster_id
            ]
    ...
    async def predict_customer_churn(self, customer_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            churn_probability = self.ml_models["churn_prediction"].predict_proba(X_pca)[
                0
            ][1]
        ...
        try:
            ...
            return {
                ...
                "growth_percentage": float(
                    (predicted_ltv - current_ltv) / current_ltv * 100
                )
                if current_ltv > 0
                else 0,
            }
    ...
    async def _get_engagement_trend(self, customer_id: int) -> float:
        ...
        try:
            ...
            recent_query = select(func.count(Order.id)).where(
                and_(
                    Order.customer_id == customer_id, Order.created_at >= recent_30_days
                )
            )
            recent_result = await self.db.execute(recent_query)
