...
const ConfigSchema = z.object({
  ...
  // CORS
  CORS_ORIGINS: z.string().transform((val: string) => 
    val ? val.split(',').map((origin: string) => origin.trim()) : ['http://localhost:3000', 'http://localhost:8000']
  ).default('http://localhost:3000,http://localhost:8000'),
  CORS_ALLOW_CREDENTIALS: z.boolean().default(true),
  CORS_ALLOW_METHODS: z.string().transform((val: string) => 
    val ? val.split(',').map((method: string) => method.trim()) : ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS']
  ).default('GET,POST,PUT,DELETE,OPTIONS'),
  CORS_ALLOW_HEADERS: z.string().transform((val: string) => 
    val ? val.split(',').map((header: string) => header.trim()) : ['*']
  ).default('*'),
  ...
  ALLOWED_FILE_TYPES: z.string().transform((val: string) => 
    val ? val.split(',').map((type: string) => type.trim()) : [
...
export function getConfig(): ConfigManager {
    ...
    switch (env) {
        case 'production':
            configManagerInstance = new ProductionConfig();
            break;
        case 'staging':
            configManagerInstance = new StagingConfig();
            break;
        default:
            configManagerInstance = new DevelopmentConfig();
            break;
    }
    return configManagerInstance;
}
