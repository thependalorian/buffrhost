...
class PropertyFinancialSettings(Base):
    ...
    buffr_commission_rate = Column(
        Numeric(5, 2), default=5.00
    )  # 5% commission to Buffr
...
class ServiceRate(Base):
    ...
    )  # room, restaurant, spa, conference, etc.
    service_category = Column(
        String(100)
    )  # accommodation, dining, wellness, business, etc.
    ...
    )  # fixed, hourly, daily, per_person, per_unit
...
class Receipt(Base):
    ...
    payment_transaction_id = Column(
        UUID(as_uuid=True), ForeignKey("payment_transaction.transaction_id")
    )
...
class ReceiptItem(Base):
    ...
    receipt_id = Column(
        UUID(as_uuid=True), ForeignKey("receipt.receipt_id"), nullable=False
    )
...
class FinancialTransaction(Base):
    ...
    )  # revenue, commission, refund, adjustment
    ...
    )  # pending, processed, settled, disputed
