...
"""
from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, Boolean, Date, Time, Numeric, Text, CheckConstraint
...
class StaffPosition(Base):
    ...
    property_id = Column(Integer, ForeignKey("hospitality_property.property_id", ondelete="CASCADE"), nullable=False)
    department_id = Column(Integer, ForeignKey("staff_department.department_id", ondelete="CASCADE"), nullable=False)
...
class StaffProfile(Base):
    ...
    staff_id = Column(String(255), ForeignKey("buffr_host_user.owner_id", ondelete="CASCADE"), primary_key=True)
    ...
    employment_type = Column(String(20), default="full_time")  # full_time, part_time, contract, temporary
    employment_status = Column(String(20), default="active")  # active, inactive, terminated, on_leave
    ...
    tasks = relationship("StaffTask", back_populates="staff", cascade="all, delete-orphan")
    performance_reviews = relationship("StaffPerformance", back_populates="staff", cascade="all, delete-orphan")
    communications = relationship("StaffCommunication", back_populates="staff", cascade="all, delete-orphan")
    leave_requests = relationship("StaffLeaveRequest", back_populates="staff", cascade="all, delete-orphan")
...
class StaffSchedule(Base):
    ...
    staff_id = Column(String(255), ForeignKey("staff_profile.staff_id", ondelete="CASCADE"), nullable=False)
    property_id = Column(Integer, ForeignKey("hospitality_property.property_id", ondelete="CASCADE"), nullable=False)
    schedule_date = Column(Date, nullable=False)
    start_time = Column(Time, nullable=False)
    end_time = Column(Time, nullable=False)
    break_duration = Column(Integer, default=30)  # minutes
    shift_type = Column(String(20), default="regular")  # regular, overtime, holiday, on_call
...
class StaffAttendance(Base):
    ...
    status = Column(String(20), default="present")  # present, absent, late, early_departure, sick_leave, vacation
    notes = Column(Text)
...
class StaffTask(Base):
    ...
    property_id = Column(Integer, ForeignKey("hospitality_property.property_id", ondelete="CASCADE"), nullable=False)
    assigned_to = Column(String(255), ForeignKey("staff_profile.staff_id", ondelete="CASCADE"), nullable=False)
    assigned_by = Column(String(255), ForeignKey("buffr_host_user.owner_id"))
    task_title = Column(String(255), nullable=False)
    task_description = Column(Text)
    task_type = Column(String(50), default="general")  # cleaning, maintenance, guest_service, kitchen, front_desk, general
    priority = Column(String(20), default="medium")  # low, medium, high, urgent
    status = Column(String(20), default="assigned")  # assigned, in_progress, completed, cancelled, overdue
...
class StaffPerformance(Base):
    ...
    punctuality_rating = Column(Integer, CheckConstraint("punctuality_rating >= 1 AND punctuality_rating <= 5"))
    work_quality_rating = Column(Integer, CheckConstraint("work_quality_rating >= 1 AND work_quality_rating <= 5"))
    teamwork_rating = Column(Integer, CheckConstraint("teamwork_rating >= 1 AND teamwork_rating <= 5"))
    customer_service_rating = Column(Integer, CheckConstraint("customer_service_rating >= 1 AND customer_service_rating <= 5"))
...
class StaffCommunication(Base):
    ...
    communication_type = Column(String(20), default="message")  # message, announcement, alert, reminder
...
class StaffLeaveRequest(Base):
    ...
    staff_id = Column(String(255), ForeignKey("staff_profile.staff_id", ondelete="CASCADE"), nullable=False)
    property_id = Column(Integer, ForeignKey("hospitality_property.property_id", ondelete="CASCADE"), nullable=False)
    leave_type = Column(String(20), nullable=False)  # vacation, sick_leave, personal, emergency, maternity, paternity
