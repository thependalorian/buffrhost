...
class VoiceModel(Base):
    ...
    provider = Column(String(50), nullable=False)  # 'openai', 'local', 'azure'
...
class VoiceInteraction(Base):
    ...
    session_id = Column(
        String(255), ForeignKey("aiagentsession.session_id", ondelete="CASCADE")
    )
...
class AudioFile(Base):
    ...
    purpose = Column(
        String(50)
    )  # 'tts_output', 'stt_input', 'background_music', 'notification'
