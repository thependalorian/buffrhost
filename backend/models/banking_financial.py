...
class (Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("profiles.id"), nullable=False) # Owner of the bank account
...
class Transaction(Base):
    ...
    type = Column(String, nullable=False) # e.g., 'deposit', 'withdrawal', 'payment', 'refund'
...
class Disbursement(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    transaction_id = Column(UUID(as_uuid=True), ForeignKey("transactions.id"), unique=True) # Link to a specific transaction if applicable
    source_account_id = Column(UUID(as_uuid=True), ForeignKey("bank_accounts.id"), nullable=False)
    destination_account_details = Column(JSONB, nullable=False) # e.g., {"bank_name": "", "account_number": ""}
    amount = Column(Float, nullable=False)
    currency = Column(String, default="NAD")
