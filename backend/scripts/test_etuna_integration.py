        ...
        if method == "GET":
            async with session.get(url) as response:
                if response.status == expected_status:
                    ...
                    print(f"❌ {method} {url} - Expected: {expected_status}, Got: {response.status}")
                    return None
        elif method == "POST":
...
async def test_etuna_integration():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        transportation_services = await test_endpoint(session, f"{base_url}/services/transportation")
        recreation_services = await test_endpoint(session, f"{base_url}/services/recreation")
        specialized_services = await test_endpoint(session, f"{base_url}/services/specialized")
        ...
        dashboard = await test_endpoint(session, f"{base_url}/admin/dashboard", expected_status=401)  # Should return 401 (unauthorized)
        reservations = await test_endpoint(session, f"{base_url}/admin/reservations", expected_status=401)
        orders = await test_endpoint(session, f"{base_url}/admin/orders", expected_status=401)
        
        # Test analytics endpoints
        revenue_analytics = await test_endpoint(session, f"{base_url}/admin/analytics/revenue", expected_status=401)
        occupancy_analytics = await test_endpoint(session, f"{base_url}/admin/analytics/occupancy", expected_status=401)
        ...
        customer_tests = [
            property_data, rooms_data, menu_data, menu_categories,
            transportation_services, recreation_services, specialized_services,
            reservation, order
        ]
        ...
        if rooms_data:
            ...
            for room in rooms_data[:3]:  # Show first 3
                print(f"  - {room.get('type_name')}: N${room.get('base_price_per_night')}/night")
