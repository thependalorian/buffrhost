...
async def test_create_employee(client: AsyncClient, auth_headers: dict, admin_user: User):
    ...
    response = await client.post("/hr-payroll/employees", json=employee_data, headers=auth_headers)
...
async def test_get_employee(client: AsyncClient, auth_headers: dict, admin_user: User):
    ...
    create_response = await client.post("/hr-payroll/employees", json=employee_data, headers=auth_headers)
    employee_id = create_response.json()["id"]

    response = await client.get(f"/hr-payroll/employees/{employee_id}", headers=auth_headers)
    assert response.status_code == 200
...
async def test_update_employee(client: AsyncClient, auth_headers: dict, admin_user: User):
    ...
    response = await client.put(f"/hr-payroll/employees/{employee_id}", json=update_data, headers=auth_headers)
    assert response.status_code == 200
...
async def test_delete_employee(client: AsyncClient, auth_headers: dict, admin_user: User):
    ...
    response = await client.delete(f"/hr-payroll/employees/{employee_id}", headers=auth_headers)
    assert response.status_code == 204

    get_response = await client.get(f"/hr-payroll/employees/{employee_id}", headers=auth_headers)
...
async def test_create_payroll_record(client: AsyncClient, auth_headers: dict, admin_user: User, db_session: AsyncSession):
    ...
    response = await client.post("/hr-payroll/payroll-records", json=payroll_data, headers=auth_headers)
    assert response.status_code == 201
