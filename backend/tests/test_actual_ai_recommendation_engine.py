...
class TestRecommendationEngine:
    ...
    def test_feature_engineering(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        assert features['dietary_match'] == 1  # Customer is vegetarian, item is vegetarian
        assert features['allergy_conflict'] == 0  # No nut allergy conflict
    
    def test_collaborative_filtering(self, recommendation_engine, sample_customer_data, sample_menu_items):
        """Test collaborative filtering recommendations."""
        # Mock similar customers
        similar_customers = [
            {'customer_id': 2, 'similarity': 0.8, 'orders': [{'menu_item_id': 1, 'rating': 5}]},
            {'customer_id': 3, 'similarity': 0.7, 'orders': [{'menu_item_id': 2, 'rating': 4}]}
        ]
        
        with patch.object(recommendation_engine, '_find_similar_customers', return_value=similar_customers):
    ...
    def test_content_based_filtering(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        vegetarian_scores = [rec['score'] for rec in recommendations if rec['item']['dietary_tags'] == 'vegetarian']
        non_vegetarian_scores = [rec['score'] for rec in recommendations if rec['item']['dietary_tags'] != 'vegetarian']
    ...
    def test_popularity_based_filtering(self, recommendation_engine, sample_menu_items):
        ...
        recommendations = recommendation_engine._popularity_based_filtering(sample_menu_items)
    ...
    def test_hybrid_recommendation(self, recommendation_engine, sample_customer_data, sample_menu_items):
    ...
    def test_dietary_preference_matching(self, recommendation_engine):
        ...
        veg_score = recommendation_engine._calculate_dietary_score(vegetarian_customer, vegetarian_item)
        meat_score = recommendation_engine._calculate_dietary_score(vegetarian_customer, meat_item)
        assert veg_score > meat_score
        
        # Vegan customer should prefer vegan items
        vegan_score = recommendation_engine._calculate_dietary_score(vegan_customer, vegan_item)
        veg_score = recommendation_engine._calculate_dietary_score(vegan_customer, vegetarian_item)
    ...
    def test_allergy_conflict_detection(self, recommendation_engine):
        ...
        safe_score = recommendation_engine._calculate_allergy_score(customer_with_nut_allergy, safe_item)
        nut_score = recommendation_engine._calculate_allergy_score(customer_with_nut_allergy, nut_item)
        dairy_score = recommendation_engine._calculate_allergy_score(customer_with_nut_allergy, dairy_item)
    ...
    def test_price_preference_matching(self, recommendation_engine):
        ...
        budget_score_budget = recommendation_engine._calculate_price_score(budget_customer, budget_item)
        budget_score_premium = recommendation_engine._calculate_price_score(budget_customer, premium_item)
        assert budget_score_budget > budget_score_premium
        
        # Premium customer should prefer premium items
        premium_score_budget = recommendation_engine._calculate_price_score(premium_customer, budget_item)
        premium_score_premium = recommendation_engine._calculate_price_score(premium_customer, premium_item)
        assert premium_score_premium > premium_score_budget
    
    def test_seasonal_recommendations(self, recommendation_engine, sample_customer_data, sample_menu_items):
        """Test seasonal recommendation adjustments."""
        # Mock current season
        with patch('ai.recommendation_engine.get_current_season', return_value='summer'):
            recommendations = recommendation_engine.get_recommendations(
                sample_customer_data, 
                sample_menu_items
            )
    ...
    def test_time_based_recommendations(self, recommendation_engine, sample_customer_data, sample_menu_items):
        """Test time-based recommendation adjustments."""
        # Mock current time
        with patch('ai.recommendation_engine.get_current_time', return_value='breakfast'):
            recommendations = recommendation_engine.get_recommendations(
                sample_customer_data, 
                sample_menu_items
            )
    ...
    def test_recommendation_diversity(self, recommendation_engine, sample_customer_data, sample_menu_items):
    ...
    def test_model_performance_metrics(self, recommendation_engine):
        ...
        # Mock test data
        test_data = [
            {'customer_id': 1, 'menu_item_id': 1, 'actual_rating': 5, 'predicted_rating': 4.5},
            {'customer_id': 2, 'menu_item_id': 2, 'actual_rating': 3, 'predicted_rating': 3.2},
            {'customer_id': 3, 'menu_item_id': 3, 'actual_rating': 4, 'predicted_rating': 4.1}
        ]
    ...
    def test_recommendation_explanation(self, recommendation_engine, sample_customer_data, sample_menu_items):
    ...
    def test_recommendation_caching(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        assert all(r1['item']['menu_item_id'] == r2['item']['menu_item_id'] for r1, r2 in zip(recommendations1, recommendations2))
    ...
    def test_error_handling(self, recommendation_engine):
        ...
        recommendations = recommendation_engine.get_recommendations(invalid_customer, [])
        
        # Should return empty list or default recommendations
        assert isinstance(recommendations, list)
        
        # Test with empty menu items
        recommendations = recommendation_engine.get_recommendations(sample_customer_data, [])
        assert isinstance(recommendations, list)
    ...
    def test_model_retraining(self, recommendation_engine):
        ...
        new_data = [
            {'customer_id': 1, 'menu_item_id': 1, 'rating': 5, 'features': [1, 2, 3, 4, 5]},
            {'customer_id': 2, 'menu_item_id': 2, 'rating': 4, 'features': [2, 3, 4, 5, 6]}
        ]
