...
class TestConversationalAI:
    ...
    def test_process_message(self, mock_process, conversational_ai):
        ...
        result = conversational_ai.process_message(
            message="Hello", customer_id=1, property_id=1
        )
...
class TestRAGSystem:
    ...
    def test_query_knowledge_base(self, mock_query, rag_system):
        ...
        result = rag_system.query_knowledge_base(
            query="What services does the hotel offer?", property_id=1
        )

        assert (
            result["answer"]
            == "Our hotel offers 24/7 room service and concierge services."
        )
...
class TestRecommendationEngine:
    ...
    def test_get_recommendations(self, mock_recommendations, recommendation_engine):
        ...
        result = recommendation_engine.get_recommendations(
            customer_id=1, property_id=1, service_type="restaurant"
        )
    ...
    @patch(
        "backend.ai.recommendation_engine.RecommendationEngine.get_cross_service_recommendations"
    )
    def test_cross_service_recommendations(
        self, mock_cross_service, recommendation_engine
    ):
        ...
        result = recommendation_engine.get_cross_service_recommendations(
            customer_id=1, property_id=1, current_service="restaurant"
        )
...
class TestLoyaltyAI:
    ...
    def test_generate_campaign(self, mock_campaign, loyalty_ai):
        ...
        mock_campaign.return_value = {
            ...
            "reward_value": 20,
            "conditions": {"min_visits": 3, "valid_days": ["saturday", "sunday"]},
            "expected_engagement": 0.75,
        }

        result = loyalty_ai.generate_campaign(
            property_id=1, campaign_type="seasonal", target_service="spa"
        )
            ...
            "segments": [
                {
                    ...
                    "customer_count": 45,
                    "criteria": {"min_total_spent": 1000, "min_visits": 5},
...
class TestAIIntegration:
    ...
    def test_ai_modules_initialization(self):
        ...
        from backend.ai import (ConversationalAI, LoyaltyAI, RAGSystem,
