...
class TestRecommendationEngine:
    ...
    def test_content_based_filtering(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        vegetarian_scores = [rec['score'] for rec in recommendations if rec['item']['is_vegetarian']]
        non_vegetarian_scores = [rec['score'] for rec in recommendations if not rec['item']['is_vegetarian']]
    ...
    def test_seasonal_recommendations(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        # Mock current season
        with patch('backend.ai.recommendation_engine.get_current_season', return_value='summer'):
            recommendations = recommendation_engine.get_recommendations(
                sample_customer_data, 
                sample_menu_items
            )
    ...
    def test_time_based_recommendations(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        # Mock current time
        with patch('backend.ai.recommendation_engine.get_current_time', return_value='breakfast'):
            recommendations = recommendation_engine.get_recommendations(
                sample_customer_data, 
                sample_menu_items
            )
    ...
    def test_model_performance_metrics(self, recommendation_engine):
        ...
        # Mock test data
        test_data = [
            {'customer_id': 1, 'item_id': 1, 'actual_rating': 5, 'predicted_rating': 4.5},
            {'customer_id': 2, 'item_id': 2, 'actual_rating': 3, 'predicted_rating': 3.2},
            {'customer_id': 3, 'item_id': 3, 'actual_rating': 4, 'predicted_rating': 4.1}
        ]
    ...
    def test_recommendation_caching(self, recommendation_engine, sample_customer_data, sample_menu_items):
        ...
        assert all(r1['item']['id'] == r2['item']['id'] for r1, r2 in zip(recommendations1, recommendations2))
