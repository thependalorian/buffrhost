...
class HospitalityPropertyFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    cuisine_type = FuzzyChoice(['international', 'local', 'italian', 'asian', 'african'])
...
class MenuCategoryFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    name = FuzzyChoice(['Appetizers', 'Main Courses', 'Desserts', 'Beverages', 'Specials'])
...
class StaffDepartmentFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    property_id = factory.SubFactory(HospitalityPropertyFactory)
    name = FuzzyChoice(['Front Desk', 'Housekeeping', 'Kitchen', 'Management', 'Maintenance'])
...
def create_test_property():
    ...
    for i in range(3):
        ...
        for j in range(5):
            MenuFactory(property_id=property.property_id, category_id=category.category_id)
    
    # Create staff structure
    department = StaffDepartmentFactory(property_id=property.property_id)
    position = StaffPositionFactory(property_id=property.property_id, department_id=department.department_id)
    StaffProfileFactory(property_id=property.property_id, department_id=department.department_id, position_id=position.position_id)
