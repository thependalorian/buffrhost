...
class TestQRLoyaltyIntegration:
    ...
    def test_generate_cross_business_redemption_qr(
        self, mock_db_session, sample_customer
    ):
    ...
    def test_scan_cross_business_redemption_qr(self, mock_db_session, sample_customer):
        ...
        with patch.object(service, "_get_customer", return_value=sample_customer):
            with patch.object(
                service.loyalty_service, "create_cross_business_redemption"
            ) as mock_redemption:
    ...
    def test_cross_business_offers_integration(self, mock_db_session, sample_customer):
        ...
        # Mock customer retrieval
        with patch.object(
            loyalty_service, "_get_customer", return_value=sample_customer
        ):
            with patch.object(
                loyalty_service, "calculate_loyalty_tier", return_value="gold"
            ):
                offers = loyalty_service.get_cross_business_offers(
                    sample_customer.customer_id, 1
                )
    ...
    def test_tier_based_benefits_application(self, mock_db_session, sample_customer):
        ...
        with patch.object(
            ...
            with patch.object(
                loyalty_service, "calculate_loyalty_tier", return_value="platinum"
            ):
                with patch.object(
                    loyalty_service, "get_tier_benefits"
                ) as mock_benefits:
