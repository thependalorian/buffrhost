...
def test_get_room_types(client: TestClient, sample_room_type):
    ...
    response = client.get(f"/api/v1/properties/{sample_room_type.property_id}/room-types/")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
def test_create_room(client: TestClient, sample_hospitality_property, sample_room_type):
    ...
    response = client.post(
        f"/api/v1/properties/{sample_hospitality_property.id}/rooms/",
        json=room_data
    )
    assert response.status_code == 201
    data = response.json()
...
def test_get_menu_categories(client: TestClient, sample_menu_category):
    ...
    response = client.get(f"/api/v1/properties/{sample_menu_category.property_id}/menu/categories/")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
def test_get_menu_items(client: TestClient, sample_menu_item):
    ...
    response = client.get(f"/api/v1/properties/{sample_menu_item.property_id}/menu/items/")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
def test_create_menu_item(client: TestClient, sample_hospitality_property, sample_menu_category):
...
def test_404_for_nonexistent_room_type(client: TestClient, sample_hospitality_property):
    ...
    response = client.get(f"/api/v1/properties/{sample_hospitality_property.id}/room-types/99999")
    assert response.status_code == 404
...
def test_booking_endpoints(client: TestClient, sample_hospitality_property, sample_room, sample_customer):
...
def test_inventory_management_endpoints(client: TestClient, sample_hospitality_property):
...
def test_analytics_endpoints(client: TestClient, sample_hospitality_property):
    ...
    response = client.get(f"/api/v1/analytics/revenue?property_id={sample_hospitality_property.id}&period=monthly")
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get(f"/api/v1/analytics/occupancy?property_id={sample_hospitality_property.id}&start_date=2024-01-01&end_date=2024-01-31")
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get(f"/api/v1/analytics/customers?property_id={sample_hospitality_property.id}")
    assert response.status_code == 200
    data = response.json()
...
def test_error_handling(client: TestClient):
    ...
    with patch('backend.routes.properties.create_property', side_effect=Exception("Database error")):
