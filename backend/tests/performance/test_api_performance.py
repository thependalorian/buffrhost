...
class TestAPIPerformance:
    ...
    def test_properties_list_performance(
        self, client: TestClient, sample_hospitality_property
    ):
    ...
    def test_property_detail_performance(
        self, client: TestClient, sample_hospitality_property
    ):
    ...
    def test_booking_creation_performance(
        self, client: TestClient, sample_customer, sample_room
    ):
    ...
    def test_database_query_performance(
        ...
        start_time = time.time()
        response = client.get(
            f"/api/v1/properties/{sample_hospitality_property.id}/analytics"
        )
        end_time = time.time()
...
class TestLoadTesting:
    ...
    def test_gradual_load_increase(self, client: TestClient):
        ...
        for num_requests in [1, 5, 10, 20, 50]:
            ...
            success_rate = sum(1 for result in results if result[1] == 200) / len(
                results
            )
