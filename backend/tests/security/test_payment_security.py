...
class TestPaymentSecurity:
    ...
    def test_payment_data_encryption(self, client: TestClient):
        ...
        assert "card_number" not in payment_info or payment_info[
            "card_number"
        ].startswith("****")
    ...
    def test_payment_refund_security(self, client: TestClient):
        ...
        refund_data = {"amount": 100.00, "reason": "Customer request"}
        ...
        response = client.post(
            "/api/v1/payments/1/refund", json=refund_data, headers=headers
        )
        assert response.status_code == 200
    ...
    def test_payment_audit_trail(self, client: TestClient):
        ...
        with patch(
            "backend.services.audit_service.log_payment_operation"
        ) as mock_audit:
            response = client.post("/api/v1/payments/", json=payment_data)
            assert response.status_code == 201
    ...
    def test_payment_webhook_security(self, client: TestClient):
        ...
        webhook_data = {"payment_id": "123", "status": "completed", "amount": 500.00}
        ...
        response = client.post(
            "/api/v1/webhooks/payment", json=webhook_data, headers=headers
        )
        assert response.status_code == 401  # Unauthorized
