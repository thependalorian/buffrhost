...
class VectorStoreManager:
    ...
    async def add_document(
        self, property_id: int, document_id: str, content: str, metadata: Dict[str, Any]
    ) -> None:
        """
    ...
    async def search(
        ...
        try:
            retriever = self.index.as_retriever(
                similarity_top_k=limit, filters={"property_id": property_id}
            )
            ...
            for result in results:
                if result.score >= similarity_threshold:
                    if document_types and result.metadata.get("document_type") not in [
                        dt.value for dt in document_types
                    ]:
                        continue
                    if tags and not any(
                        tag in result.metadata.get("tags", []) for tag in tags
                    ):
                        continue
            ...
            logger.info(
                f"Found {len(filtered_results)} matching documents for query: {query}"
            )
    ...
    async def get_similar_documents(
        self, property_id: int, document_id: str, limit: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            async with self.db_session.begin():
                result = await self.db_session.execute(
                    text(
                        "SELECT COUNT(*) FROM knowledge_embeddings WHERE (metadata_->>'property_id')::int = :property_id"
                    ),
                    {"property_id": property_id},
                )
