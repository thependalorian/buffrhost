...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        if self.llama_parse_api_key:
            ...
            logger.warning("LLAMA_PARSE_API_KEY not found. Using fallback document processing.")
    ...
    async def process_file(
        ...
        try:
            ...
            processed_content = await self.process_document(content, document_type, metadata)
    ...
    async def _process_docx(self, file_path: Path) -> str:
        ...
        try:
            ...
            return f"[DOCX content from {file_path.name} - requires python-docx library]"
        except Exception as e:
    ...
    async def _process_pptx(self, file_path: Path) -> str:
        ...
        try:
            ...
            # For now, return a placeholder
            return f"[PPTX content from {file_path.name} - requires python-pptx library]"
        except Exception as e:
    ...
    async def extract_key_information(
        ...
        try:
            ...
            extracted_info["key_phrases"] = list(set(key_phrases))[:20]  # Top 20 unique phrases
    ...
    async def process_document_with_llama_index(
        ...
        try:
            ...
            if self.llama_parser and file_path.suffix.lower() in ['.pdf', '.docx', '.pptx']:
                markdown_content = await self._process_with_llama_parse(file_path)
            else:
                # Fallback to standard processing
                markdown_content = await self.process_file(str(file_path), document_type, metadata)
            ...
            for node in nodes:
                if len(node.text) > 2000:  # If node is too large, split further
                    sentence_nodes = self.sentence_splitter.get_nodes_from_documents([node])
            ...
            logger.info(f"Successfully processed {file_path} into {len(final_nodes)} chunks")
            return result
