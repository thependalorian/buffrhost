...
from .knowledge_base import KnowledgeBaseManager, DocumentType
...
class RAGResponse(BaseModel):
    ...
    requires_human: bool = Field(default=False, description="Whether human assistance is needed")
...
class RAGAgent:
    ...
    async def _classify_query(self, question: str) -> QueryType:
        ...
        # Emergency keywords
        if any(word in question_lower for word in ["emergency", "urgent", "help", "fire", "medical", "police"]):
            return QueryType.EMERGENCY
        
        # Policy keywords
        if any(word in question_lower for word in ["policy", "rule", "regulation", "allowed", "prohibited"]):
            return QueryType.POLICY_QUESTION
        
        # Procedure keywords
        if any(word in question_lower for word in ["how to", "procedure", "process", "steps", "guide"]):
            return QueryType.PROCEDURE_QUESTION
        
        # Contact keywords
        if any(word in question_lower for word in ["contact", "phone", "email", "address", "location"]):
            return QueryType.CONTACT_INFO
        
        # Booking keywords
        if any(word in question_lower for word in ["book", "reservation", "check-in", "check-out", "room"]):
            return QueryType.BOOKING_HELP
        
        # Service keywords
        if any(word in question_lower for word in ["service", "amenity", "facility", "restaurant", "spa"]):
            return QueryType.SERVICE_INFO
        
        # FAQ keywords
        if any(word in question_lower for word in ["what", "when", "where", "why", "how", "?"]):
    ...
    async def _retrieve_relevant_documents(
        ...
        try:
            ...
            document_type_mapping = {
                ...
                QueryType.BOOKING_HELP: [DocumentType.BOOKING_PROCEDURES, DocumentType.ROOM_INFO],
    ...
    async def get_suggested_questions(
        ...
        try:
            ...
            summary = await self.knowledge_base.get_property_knowledge_summary(property_id)
