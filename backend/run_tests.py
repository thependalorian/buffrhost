...
def main():
    ...
    if not run_command("python -m flake8 backend/ --max-line-length=100 --ignore=E203,W503", "Code linting"):
    ...
    if not run_command("python -m pytest tests/ --cov=backend --cov-report=html --cov-report=term", "Tests with coverage"):
        print("❌ Coverage tests failed")
        sys.exit(1)
    
    # Run AI module tests specifically
    print("\n🤖 Running AI module tests...")
    if not run_command("python -m pytest tests/test_ai_modules.py -v -m ai", "AI module tests"):
        print("❌ AI module tests failed")
        sys.exit(1)
    
    # Run database model tests
    print("\n🗄️  Running database model tests...")
    if not run_command("python -m pytest tests/test_database_models.py -v", "Database model tests"):
        print("❌ Database model tests failed")
        sys.exit(1)
    
    # Run API endpoint tests
    print("\n🌐 Running API endpoint tests...")
    if not run_command("python -m pytest tests/test_api_endpoints.py -v", "API endpoint tests"):
